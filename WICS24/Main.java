    ...
    private static void readData() throws IOException{
        ...
        try(BufferedReader sc = new BufferedReader(new FileReader(filepath))) {
            ...
            while ((line = sc.readLine()) != null) {
                Attack temp = new Attack(line.split(","));
                allAttacks.put(temp.getName(), temp);
            }
        ...
        try(BufferedReader sc = new BufferedReader(new FileReader(filepath))) {
            ...
            while ((line = sc.readLine()) != null) {
                list = line.split(",");
                String name = list[0];
                ArrayList<Attack> tempAttacks = new ArrayList<>();
                for (int i = 1; i < list.length; i++) {
                    tempAttacks.add(allAttacks.get(name));
                }
                NPCs.get(name).setAttacks(tempAttacks);
            }
        } catch (FileNotFoundException e) {
            System.out.println("Where is the file dude. YOU HAD ONE JOB");
        }
